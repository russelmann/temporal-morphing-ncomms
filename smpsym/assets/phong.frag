...
void main()
{
	...
	//float phong = max( dot( N, L ), 0.4 );
	float phong = max( abs( dot( N, L ) ), 0.4 ); // equal for opposite sides

	const float kMaterialShininess = 7.5;
	...
	float blinn = pow( max( abs( dot( N, H ) ), 0.0 ), kMaterialShininess ) * kNormalization; // equal for opposite sides
	...
	if( uTexturingMode == 1 ) {
		...
		diffuse *= texture( uTex0, vVertexIn.texCoord.st / uFreq ).rgb;
	...
	// ambient
	vec3 ambient = cDiffuse;//vec3(0.3, 0.3, 0.3);

	// final color
	oFragColor = vec4( ambient * 0.4 + diffuse * 0.5 + specular, alpha );
}
