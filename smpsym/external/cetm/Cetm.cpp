...
// get scaling factors
bool Cetm::get_scaling_factors(Eigen::VectorXd& u) const
{
	u.resize(mesh.vertices.size());
	int i = 0;
	for (auto v = mesh.vertices.begin(); v != mesh.vertices.end(); ++v)
	{
		u[i++] = v->isBoundary() ? 0.0 : solver.x[index.at(v->index)];
	}
	return true;
}
