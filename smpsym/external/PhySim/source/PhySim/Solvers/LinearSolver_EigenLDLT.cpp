...
namespace PhySim
{
	...
	LinearSolver_EigenLDLT::LinearSolver_EigenLDLT(const MatrixSd& mA, const LinearSolverOptions& options) : LinearSolver(mA, options)
	{
	...
	void LinearSolver_EigenLDLT::Init(const MatrixSd& mA, const LinearSolverOptions& options)
	{
	...
	void LinearSolver_EigenLDLT::Free()
	{
		// Nothing to do here...
	}

	SolveResult LinearSolver_EigenLDLT::SolveInternal(MatrixSd& mA, const VectorXd& vb, VectorXd& vx)
	{
		int N = (int) mA.rows();
		int M = (int) mA.cols();
		...
		for (i = 0; i < this->m_options.regIters; ++i)
		{
			...
			for (int j = 0; j < N; ++j)
			{
				...
				{
					logSimu("\n[FAILURE] Linear solve: singular matrix, value: %.9f", vd(j));
					jumpSin = true;
					break;
				}
			}
			...
			logSimu("\n[SUCCESS] Linear solve: solved using Eigen LDLT. Reg: %d", i);
	...
	SolveResult LinearSolver_EigenLDLT::SolveInternal(MatrixSd& mA, const MatrixXd& mB, MatrixXd& mX)
	{
		assert(false);
	...
	SolveResult LinearSolver_EigenLDLT::SolveInternal(MatrixSd& mA, const MatrixSd& mB, MatrixSd& mX)
	{
		assert(false);
	...
	//SolveResult LinearSolver_EigenLDLT::Solve_CG(MatrixSd& mA, const VectorXd& vb, VectorXd& vx)
	//{
	//	int N;
	//	Real normA;
	//	MatrixSd mR;
	...
	//SolveResult LinearSolver_EigenLDLT::Solve_LU(MatrixSd& mA, const VectorXd& vb, VectorXd& vx)
	//{
	//	int N;
	//	Real normA;
	//	MatrixSd mR;
