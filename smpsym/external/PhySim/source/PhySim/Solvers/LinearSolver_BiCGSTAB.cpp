...
namespace PhySim
{
	...
	LinearSolver_BiCGSTAB::LinearSolver_BiCGSTAB(const MatrixSd& mA, const LinearSolverOptions& options) : LinearSolver(mA, options)
	{
	...
	void LinearSolver_BiCGSTAB::Init(const MatrixSd& mA, const LinearSolverOptions& options)
	{
	...
	void LinearSolver_BiCGSTAB::Free()
	{
		// Nothing to do here...
	}

	SolveResult LinearSolver_BiCGSTAB::SolveInternal(MatrixSd& mA, const VectorXd& vb, VectorXd& vx)
	{
		MatrixSd mAFull = mA.selfadjointView<Lower>();
		...
		for (i = 0; i < this->m_options.regIters; ++i)
		{
			...
			logSimu("\n[SUCCESS] Linear solve: solved using BiCGSTAB. Reg: %d, Iter: %d, Error: %f", i, m_solver.iterations(), m_solver.error());
	...
	SolveResult LinearSolver_BiCGSTAB::SolveInternal(MatrixSd& mA, const MatrixXd& mB, MatrixXd& mX)
	{
      assert(false);
	...
	SolveResult LinearSolver_BiCGSTAB::SolveInternal(MatrixSd& mA, const MatrixSd& mB, MatrixSd& mX)
	{
		assert(false);
	...
	//SolveResult LinearSolver_BiCGSTAB::Solve_LU(MatrixSd& mA, const VectorXd& vb, VectorXd& vx)
	//{
	//	int N;
	//	Real normA;
	//	MatrixSd mR;
